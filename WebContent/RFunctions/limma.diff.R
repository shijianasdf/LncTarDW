limma.diff <- function(filepath,lncRNA,target){
  ## 读入数据
  ...
  if(lncRNA != "" & target != ""){
    ## 差异表达表格 返回diff.table
    lncRNA.diff <- round(DEG[rownames(DEG) == lncRNA,c("AveExpr","logFC","adj.P.Val")],digits = 2)
    target.diff <- round(DEG[rownames(DEG) == target,c("AveExpr","logFC","adj.P.Val")],digits = 2)
    colnames(lncRNA.diff) <- c("baseMean","log2FoldChange","padj")
    colnames(target.diff) <- c("baseMean","log2FoldChange","padj")
